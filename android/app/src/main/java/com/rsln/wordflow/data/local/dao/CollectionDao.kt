...
interface CollectionDao {
    ...
    suspend fun findByRemoteId(remoteId: String): CollectionEntity?

    @Query("UPDATE collections SET remoteId = :remoteId, updatedAt = :updatedAt WHERE id = :id")
    suspend fun setRemoteId(id: Long, remoteId: String, updatedAt: Long = System.currentTimeMillis())

    @Query("DELETE FROM collections WHERE remoteId = :remoteId")
    suspend fun deleteByRemoteId(remoteId: String)

    @Query("DELETE FROM collections")
    suspend fun deleteAllCollections()

    @Query("DELETE FROM word_collection_cross_ref")
